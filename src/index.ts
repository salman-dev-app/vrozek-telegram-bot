 ...
 * One public webhook: Telegram updates (POST /webhook). No store/order endpoints.
 */
...
app.post('/webhook', async (c) => {
  ...
  const tg = new TgClient(env.TELEGRAM_BOT_TOKEN, (m) =>
    c.executionCtx.waitUntil(logEvent(env, 'tg', m).catch(() => {}))
  );
  try {
    if (update.message) {
